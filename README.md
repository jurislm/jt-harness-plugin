...
- Codex 与 CodeRabbit 各进行一次完整审查。Codex 的唯一审查使用 `requesting-code-review` 作为 PR 前清单；PR 开启时自动启动后，等待其完成并返回结果，再计为这一次。若没有自动审查启动，按清单手动请求一次。若唯一一次 Codex 审查失败或无结果，记录情况，不发起替代审查；审查要求仍未满足时不合并。CodeRabbit 自动审查启动时等待结果，不重复提及 `@CodeRabbit`；没有自动运行时手动请求一次，PR 管道受限时再用 CLI 一次。两种 CodeRabbit 管道都受限时依赖已完成的 Codex 审查。提交修正后只检查差异并验证修正，不触发增量或第二次完整审查。
