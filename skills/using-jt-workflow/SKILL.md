...
description: >
  JT Harness 的紀律與 Skill 選用：產品團隊心智模型、可替換工具與具名依賴的判別、
...
| 要做的事 | 用什麼 |
|---|---|
| 問題、產品需求、開發任務與專案的建立、整理、關聯 | `product-management` |
| 一件工程案件的端到端交付 | `engineering-delivery` |
| 工法（釐清、TDD、除錯、審查、驗收、worktree、合併） | `superpowers:*` 對應的 Skill |
| 產品需求與規格 | Notion `功能要求` 的產品需求、`工程文件` 的已發布規格 |
| 工程任務、進度、證據 | Notion `功能要求` 的開發任務與其留言 |
| 問題與專案 | Notion `問題追蹤`、`專案` |

案件管理走 JT Harness，工法走 superpowers。四個 Notion 資料庫各有權責：
`工程文件` 的已發布頁面記產品規格；`功能要求` 以 `記錄類型` 區分產品需求與開發任務；
`問題追蹤` 記來源問題；`專案` 記排程與優先順序。開發任務用 `對應需求` 連結一或多筆產品需求，
用 `來源問題` 連結問題；一個需求拆成多個任務時，需求頁的 `開發任務` 反向關聯列出各任務。

## 具名依賴 vs 可替換工具

| 類別 | 定義 | 例子 | 不可用時 |
|---|---|---|---|
| 具名依賴 | 它就是方法或關卡本身，沒有等價替代品 | Notion 工作區、`git`、`superpowers:*` 各工法、`coderabbit:code-review` | 走該處明訂的出口，不尋找替代 |
| 可替換工具 | 只是取得某個事實的一種管道 | Notion MCP 或介面、`gh`、GitHub MCP | 換另一個能取得同一事實的管道，不因此停下 |
...
2. **repo 事實去讀該 repo 自己宣告的定義。** 驗證指令、merge gate 清單、hook 行為、
   ...
   上一級無宣告才往下一級取，不跳級。
3. **Notion 是案件檔案。** 每個節點結束、每次停下，都寫回同一開發任務。
...
| 「等全部做完再寫回 Notion」 | 案件記錄是過程，不是結尾 |

## 兩條工作線

產品／專案資料與任務準備歸 `product-management`；已有開發規格的任務執行歸
`engineering-delivery`。一個請求同時涵蓋兩者時先完成產品需求與任務 readback，
再把同一任務頁交給工程流程。
