...
- Use `superpowers:requesting-code-review` for one complete Codex review. If the repository starts a Codex PR review automatically, wait for it to finish and return a result before counting it as that review. If it fails before returning a result, use `requesting-code-review` once.
- Request one CodeRabbit review on the PR using `@CodeRabbit` and wait for its result. If the PR route is limited, try the CodeRabbit CLI once. If both routes are limited, record the limits and rely on the completed Codex review.
