...
- Use `superpowers:requesting-code-review` as the pre-review checklist for one complete Codex review. If the repository starts a Codex PR review automatically, wait for it to finish and return a result before counting it as that review. If no automatic review starts, request one once after the checklist. If the single Codex review fails or returns no result, record that outcome and do not request a replacement. The Codex review requirement remains unmet; do not merge while a required review gate is unsatisfied.
